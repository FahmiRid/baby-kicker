...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-8 bg-[#FFF5F5]">
      ...
      {/* Auth Card */}
      <div className="w-full max-w-md bg-white rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(255,129,141,0.1)] mb-8 relative overflow-hidden">
        {/* Error Display */}
        {error && (
          <div className="absolute top-0 left-0 right-0 bg-[#FFE6E9] py-3 px-4 text-[#FF818D] text-xs font-bold text-center animate-in slide-in-from-top duration-300">
            {error}
          </div>
        )}
        ...
        <button 
          ...
          className="w-full flex items-center justify-center gap-3 py-4 border border-[#F0F0F0] rounded-full text-[#4A4A4A] font-medium hover:bg-gray-50 transition-colors mb-8 mt-4"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05" />
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 12.16-4.53z" fill="#EA4335" />
          </svg>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-5 bg-[#FF818D] text-white font-bold rounded-full shadow-[0_10px_25px_rgba(255,129,141,0.4)] hover:scale-[1.02] active:scale-[0.98] transition-all mt-4 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
